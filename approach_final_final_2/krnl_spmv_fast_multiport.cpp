...
#pragma HLS ARRAY_PARTITION variable=x_dup dim=1
	...
	int row_length_pad = 0, row_length = 0, row_counter = 0;
...
#pragma HLS stream variable=values_fifo type=fifo depth=32
...
#pragma HLS stream variable=cols_fifo type=fifo depth=32
...
#pragma HLS stream variable=results_fifo type=fifo depth=32
...
#pragma pipeline off
			new_nnz_split += rows_length_pad[l];
			nnz_split += rows_length[l];
		}

		{
#pragma HLS DATAFLOW
			for(int j = 0; j < new_nnz_split; j++) {
#pragma HLS pipeline off
				if (j < nnz_split) {
					values_fifo << values[index];
					cols_fifo << cols[index++];
				}
			}
...
//		cols_fifo   << cols[i];
//	}
		Loop_rows: for (int i = 0; i < NUM_ROWS; i++) {
			Loop_onerow: for (int n = 0; n < rows_length_pad[i]; n+=II) {

				#pragma HLS PIPELINE
				if (row_length_pad == 0) {
					row_length_pad = rows_length_pad[i];
					row_length = rows_length[i];
					row_counter = 0;
					sum = 0;
				}
...
#pragma HLS unroll off
		spmv_kernel(rows_length, rows_length_pad, cols, values, y, x, cnt);
	}
	y[0] += cnt[0];

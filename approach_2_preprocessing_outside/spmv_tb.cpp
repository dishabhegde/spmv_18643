...
int rows_length_pad[NUM_ROWS] = {0};
int new_nnz = 0;
...
//            double randValue = (double)rand() / RAND_MAX * (2 / (1 + exp(-0.002 * (float)(NNZ - index))) - 1);
...
//    DTYPE *values = (DTYPE*) malloc(NNZ * sizeof(DTYPE));
    ...
    OCL_CHECK(err, cl::Buffer buffer_rows_length(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, (NUM_ROWS) * sizeof(int), rows_length, &err));
    OCL_CHECK(err, cl::Buffer buffer_rows_length_pad(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, (NUM_ROWS) * sizeof(int), rows_length_pad, &err));
    OCL_CHECK(err, cl::Buffer buffer_y(context, CL_MEM_WRITE_ONLY, SIZE * sizeof(DTYPE), NULL, &err));

    //set the kernel Arguments
    int narg=0;
    OCL_CHECK(err, err = krnl_sparse_matrix_mul.setArg(narg++,buffer_rows_length));
    OCL_CHECK(err, err = krnl_sparse_matrix_mul.setArg(narg++,buffer_rows_length_pad));
...
//	std::cout << std::endl;
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_rows_length,buffer_rows_length_pad,buffer_columnIndex,buffer_values,buffer_x},0/* 0 means from host*/));
